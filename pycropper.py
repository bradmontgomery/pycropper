''' 
    ...
    Author: 
        This software is Copyright 2008 by Brad Montgomery and was 
        originally available from <http://bradmontgomery.net/pycropper/>

    License: (see file COPYING) included with this program.
...
screen_height = 480 # height of the screen
...
def load_image_list(directory):
    ''' I create a list of image filenames from a directory 
        ignoring any subdirectories or files that start with a "."
    '''
    images = os.listdir(directory)
    print 'list of images: ', images
    images.sort() # sort the images in alpha-num order (assume filenames like snapshot00.pgm...)
    i = 0
    for image in images:
        filename, ext = os.path.splitext(image)
        if ext in supported_files:
            if os.path.isfile(os.path.join(directory, image)) and image[0] != ".":
                images[i] = image
                print 'keeping ', image
                i = i + 1

    images = images[:i] # make sure we don't have any duplicates (in case we skipped 
                        # something like .svn)
    return images 

def cropImage(im, left, top, right, bottom):
    ''' Crop the given PIL image inside the given coordinates. 
...
def PIL_to_surface(im):
    ...
    return  pygame.image.fromstring(im.tostring(), im.size, im.mode, flipped=False)
...
def draw_welcome(screen):
    ...
    #line1 = "pyCropper"
    #titlefont = pygame.font.SysFont("courier new", 44, True)
    line2 = "when you need to do a lot of quick cropping." 
    instructions = ["all images in the current directory will be loaded, ",
                    ...
                    "",
                    "pyCropper is licensed unter the terms of the GPLv3"]
    line3 = "(press Space to continue)"

    #line1_surface= titlefont.render(line1, True, (255,55,55))
    ...
    #screen.blit(line1_surface, (100,10))
...
def input(events):
    ...
    global click_state, current_rect, crop_rect, current_file_index, files, reloadImage
    ...
    for event in events:
        ...
        elif event.type == MOUSEBUTTONDOWN and event.button == 1 and click_state == 0: # left mouse click
            click_state = 1 # Stat of left-click: 
            current_rect = pygame.Rect(event.pos[0], event.pos[1], 1, 1) # left, top, width, height
        elif event.type == MOUSEBUTTONDOWN and event.button == 1 and click_state == 1: # left mouse click
            click_state = 2 # Stat of left-click: 
            crop_rect = pygame.Rect(current_rect.left, current_rect.top, current_rect.width, current_rect.height)
            current_rect = None
        elif event.type == MOUSEBUTTONDOWN and event.button == 3 and click_state == 1: # right mouse click
            # Cancel the Rect
            current_rect = None
            click_state = 0
        elif event.type == MOUSEMOTION and click_state == 1:
...
def main():
    global files, screen_width, screen_height, resolution, current_rect, crop_rect, click_state, current_file_index, reloadImage, wait_time
    ...
    while current_file_index < len(files):
        ...
        if click_state == 0:
            screen.fill((0,0,0))
            screen.blit(image, (0,0)) # draw the image
            txt = "Showing Image #%s: %s"%(current_file_index, files[current_file_index])
            msg = text_image(txt, 24, (255,255,255))
            screen.blit(msg, (10, screen_height - 30))
        ...
        if click_state == 1: 
            ...
            # crop the image 
            #crop_image = image.subsurface(crop_rect).convert_alpha() # the convert alpha get the image data.
            ...
            if current_file_index < len(files):
               ...
               reloadImage = True
                ...
                image = None
            pygame.display.flip()
            sleep(wait_time) # pause just a second
